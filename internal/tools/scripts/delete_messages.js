function run(argv) {
  ...
  if (!accountName || !mailboxPath || !messageIds) {
    return JSON.stringify({
      success: false,
      error: "account, mailboxPath and messageIds are required.",
  ...
  try {
    ...
    log(
      `Deleting ${messageIds.length} message(s) from "${mailboxPath.join(" > ")}"${dryRun ? " (dry run)" : ""}.`,
    );
    ...
    for (let i = 0; i < messageIds.length; i++) {
      ...
      try {
        // Mail.delete moves the message to the account's Trash (Deleted Messages);
        // it is not permanently removed until the Trash is emptied.
        Mail.delete(msg);
        succeeded.push({ id: id, subject: subject, sender: sender });
      } catch (e) {
        log(`Delete failed for ID ${id}: ${e.toString()}`);
        failed.push({ id: id, error: e.toString() });
      }
    }

    log(
      `Done: ${succeeded.length} ${dryRun ? "would be deleted" : "deleted"}, ${notFound.length} not found, ${failed.length} failed.`,
    );
    ...
    return JSON.stringify({
      ...
      data: {
        ...
        deleted: dryRun ? [] : succeeded,
        would_delete: dryRun ? succeeded : [],
        not_found: notFound,
        failed: failed,
        message: dryRun
          ? `Dry run: ${succeeded.length} of ${messageIds.length} message(s) would be moved to Trash.`
          : `${succeeded.length} of ${messageIds.length} message(s) moved to Trash.`,
      },
      logs: logs.join("\n"),
    });
  } catch (e) {
    log(`Error deleting messages: ${e.toString()}`);
    return JSON.stringify({
      success: false,
      error: `Failed to delete messages: ${e.toString()}`,
      logs: logs.join("\n"),
    });
  }
}
